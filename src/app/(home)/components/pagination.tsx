...
function PageButton({ className, ...props }: ComponentProps<'button'>) {
  return <button className={clsx(["cursor-pointer hover:shadow-sm hover:shadow-slate-700/30 text-xs md:text-base md:leading-6 font-bold flex items-center justify-center md:px-3 md:py-2 p-2 data-[active='true']:bg-slate-700 dark:data-[active='true']:bg-sky-400 bg-gray-400 dark:bg-slate-400 text-white rounded-sm", className])} {...props} />
}
